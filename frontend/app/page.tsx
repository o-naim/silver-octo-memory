...
const AGENT_META: Record<AgentId, { label: string }> = {
  normalizer:    { label: "Normalizer"           },
  surface_enum:  { label: "Surface Enumerator"   },
  exfil_analyst: { label: "Exfiltration Analyst" },
  nist_mapper:   { label: "NIST Mapper"          },
  csv_builder:   { label: "CSV Builder"          },
}
...
function safeStr(v: unknown): string {
  ...
  if (typeof v === "object") {
    const vals = Object.values(v as Record<string, unknown>).flat()
    if (vals.length) return vals.map((x) => safeStr(x)).join(", ")
...
export default function Page() {
  ...
  const handleDownloadCSV = () => {
    ...
    const headers = ["environnement", "surface", "detail_de_surface", "technique_exploitee", "explication_technique"]
    const rows = [
      headers.join(","),
                      ...
                      <div className="flex flex-wrap gap-2 mt-1">
                        {Object.entries(ag.stats).map(([k, v]) => (
                          <span key={k} className="text-[10px] bg-[#1a1a24] border border-[#1e2030] px-1.5 py-0.5 rounded">
                            ...
                            <span className="text-[#00ff9d]">{safeStr(v)}</span>
                          </span>
                        ))}
                      </div>
                    )}
        ...
        {result && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                { label: "Vecteurs identifiés", value: String(result.total_vectors) },
                { label: "Familles NIST",       value: safeStr(result.familles_nist) },
                { label: "Avertissements",      value: String(result.warnings?.length ?? 0) },
              ].map(({ label, value }) => (
                <div key={label} className="border border-[#1e2030] bg-[#111115] rounded p-3">
                  ...
                  <div className="text-sm text-[#00ff9d] font-medium truncate">{value}</div>
                </div>
              ))}
            </div>
            ...
            {result.resume_nist && (
              <div className="border border-[#1e2030] bg-[#111115] rounded p-4">
                ...
                <p className="text-xs text-[#94a3b8] leading-relaxed">{safeStr(result.resume_nist)}</p>
              </div>
            )}
                ...
                <div className="overflow-x-auto max-h-80 overflow-y-auto">
                  <table className="w-full text-[10px]">
                    <thead className="sticky top-0 bg-[#0d0d0f]">
                      <tr>
                        {["Environnement", "Surface", "Détail surface", "Technique", "Explication"].map((h) => (
                    ...
                    <tbody>
                      {result.csv_rows.slice(0, 20).map((row, i) => (
                        <tr key={i} className={i % 2 === 0 ? "bg-[#0d0d0f]" : "bg-[#111115]"}>
                          <td className="px-3 py-2 text-[#4a5568] whitespace-nowrap max-w-[110px] truncate">{row.environnement}</td>
                          <td className="px-3 py-2 text-[#00ff9d] max-w-[120px] truncate">{row.surface}</td>
                          <td className="px-3 py-2 text-[#94a3b8] max-w-[150px] truncate">{row.detail_de_surface}</td>
                          <td className="px-3 py-2 text-[#00ff9d]/70 max-w-[150px] truncate">{row.technique_exploitee}</td>
                          <td className="px-3 py-2 text-[#64748b] max-w-[240px] truncate">{row.explication_technique}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
